...
describe('VRFCoordinatorV2', () => {
  ...
  beforeEach(async () => {
    ...
    pliToken = await ltFactory.deploy()
    ...
    mockPliEth = await mockAggregatorV3Factory.deploy(0, pliEth)
    ...
    c = {
      ...
      weiPerUnitPli: BigNumber.from('10000000000000000'),
    }
  ...
  describe('#setConfig', async function () {
    ...
    it('positive fallback price', async function () {
      await expect(
        ...
        .to.be.revertedWithCustomError(vrfCoordinatorV2, 'InvalidPliWeiPrice')
        .withArgs(0)
      await expect(
        vrfCoordinatorV2
          .connect(owner)
  ...
  describe('#cancelSubscription', async function () {
    ...
    it('can cancel', async function () {
      ...
      const randomBalance = await pliToken.balanceOf(randomAddress)
  ...
  describe('#recoverFunds', async function () {
    ...
    it('owner can recover pli transferred', async function () {
      // Set the internal balance
      assert(BigNumber.from('0'), pliToken.balanceOf(randomAddress))
      const s = ethers.utils.defaultAbiCoder.encode(['uint64'], [subId])
      await pliToken
        .connect(subOwner)
      ...
      assert(BigNumber.from('1000'), pliToken.balanceOf(randomAddress))
    })
  })
  ...
  it('subscription lifecycle', async function () {
    ...
    const random2Balance = await pliToken.balanceOf(randomAddress)
  ...
  describe('#requestRandomWords', async function () {
    ...
    it('gas limit too high', async function () {
      await pliToken.connect(subOwner).transferAndCall(
        vrfCoordinatorV2.address,
  ...
  describe('#calculatePaymentAmount [ @skip-coverage ]', async function () {
    it('output within sensible range', async function () {
      ...
      // The gas price is 1gwei and the eth/pli price is set to 300000000 wei per unit pli.
      // paymentAmount = 1e18*weiPerUnitGas*(gasAfterPaymentCalculation + startGas - gasleft()) / uint256(weiPerUnitPli);
    ...
    it('payment too large', async function () {
      ...
      // 1e18*1e18/30000000
      // 3.333333333E28 > 1e27 (all pli in existence)
      await expect(
    ...
    it('non-positive pli wei price should revert', async function () {
      const mockAggregatorV3Factory = await ethers.getContractFactory(
        'src/v0.8/tests/MockV3Aggregator.sol:MockV3Aggregator',
        owner,
      )
      ...
      const mockPliEthZero = await mockAggregatorV3Factory.deploy(0, 0)
      const vrfCoordinatorV2TestHelperZero =
        await vrfCoordinatorV2TestHelperFactory.deploy(mockPliEthZero.address)
      await expect(
        ...
        .to.be.revertedWithCustomError(vrfCoordinatorV2, `InvalidPliWeiPrice`)
        .withArgs(0)
      const mockPliEthNegative = await mockAggregatorV3Factory.deploy(0, -1)
      const vrfCoordinatorV2TestHelperNegative =
        await vrfCoordinatorV2TestHelperFactory.deploy(
          mockPliEthNegative.address,
        )
      await expect(
