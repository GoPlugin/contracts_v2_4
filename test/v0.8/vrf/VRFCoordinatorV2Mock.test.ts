...
describe('VRFCoordinatorV2Mock', () => {
  ...
  let pointOnePli = BigNumber.from('100000000000000000')
  let onePli = BigNumber.from('1000000000000000000')
  let keyhash =
  ...
  describe('#fundSubscription', async function () {
    it('can fund a subscription', async function () {
      let subId = await createSubscription()
      await expect(
        vrfCoordinatorV2Mock.connect(subOwner).fundSubscription(subId, onePli),
      )
        .to.emit(vrfCoordinatorV2Mock, 'SubscriptionFunded')
        .withArgs(subId, 0, onePli)
      let sub = await vrfCoordinatorV2Mock
        .connect(subOwner)
        .getSubscription(subId)
      expect(sub.balance).to.equal(onePli)
    })
    it('cannot fund a nonexistent subscription', async function () {
      await expect(
        vrfCoordinatorV2Mock.connect(subOwner).fundSubscription(4, onePli),
      ).to.be.revertedWithCustomError(
        vrfCoordinatorV2Mock,
        'InvalidSubscription',
      )
  ...
  describe('#fulfillRandomWords', async function () {
    ...
    it('can request and fulfill [ @skip-coverage ]', async function () {
      ...
      expect(sub.balance).to.equal(
        onePli.sub(receipt.events[0].args['payment']),
      )
    })
