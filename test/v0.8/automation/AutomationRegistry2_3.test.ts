...
describe('AutomationRegistry2_3', () => {
  ...
  describe('#withdrawOwnerFunds', () => {
    it('can only be called by finance admin', async () => {
      await evmRevertCustomError(
        registry.connect(keeper1).withdrawPli(zeroAddress, 1),
        registry,
        'OnlyFinanceAdmin',
      )
    })
    ...
    itMaybe('withdraws the collected fees to owner', async () => {
      ...
      let ownerRegistryBalance = await registry.pliAvailableForPayment()
      assert.isTrue(ownerRegistryBalance.eq(upkeepBalance))

      // Now withdraw
      await registry
        .connect(financeAdmin)
        .withdrawPli(await owner.getAddress(), ownerRegistryBalance)

      ownerRegistryBalance = await registry.pliAvailableForPayment()
